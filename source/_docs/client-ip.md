...
When using Cloudflare in combination with the Domain Access module on Drupal 7, the user's IP address will get cached by the `ip_address()` function incorrectly very early during the bootstrap process. To correct this you can add similar code to your `settings.php` file, above where you include the Domain Access module's `settings.inc` file.
